...
function NavBar() {
  ...
  // const getMyInitials = () =>{
  //   let firstLetter = user.first_name[0]
  //   let lastLetter = user.last_name[0]
            ...
            <Box>
              <Tooltip title="Open settings">
                <IconButton onClick={handleOpenUserMenu} sx={{ p: 0 }}>
                  {(getInitials(user)) && <Avatar src={user.image_path} alt={initials} sx={{ bgcolor: 'secondary' }} />}
                  {/* { getInitials(user) && <Avatar sx={{ bgcolor: 'secondary' }}>{initials}</Avatar>} */}
                </IconButton>
              </Tooltip>
              <Menu
                sx={{ mt: '45px' }}
      ...
      <AppBar position="fixed" color="primary" sx={{ top: 'auto', bottom: 0 }}>
        <Toolbar>
          <IconButton>
            ...
            {user.PTO > 13 ?
              <Typography color='white'>{user.PTO}hrs PTO remain</Typography>
              :
              <Typography color='error' fontWeight='bold' >{user.PTO}hrs PTO remain</Typography>
            }
